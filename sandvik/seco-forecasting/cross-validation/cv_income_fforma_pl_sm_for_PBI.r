...
# MAGIC %run /Repos/alejandro.kuratomi_hernandez@secotools.com/prophet_R_secotools/sandvik/seco-forecasting/read_data/income/income_read_productline_sm
...
create_temporal_list <- function(data){
  ...
  data %>%
    crossing(cut_off) %>%
    group_split(ProductLine, SalesMarket, cut_off) %>%
    lapply(., to_list)
}
...
##Aug and sep 2022 are outliers
L_index <- c()
for(i in 1:length(temporal_list)){
  L <- temporal_list[[i]]$L
  xx <- temporal_list[[i]]$Date %>% tail(1) %m+% months(1)
  aug_2022 <- "2022-08-01" %>% as.Date()
  sep_2022 <- "2022-09-01" %>% as.Date()
  if( L < 25) L_index <- c(L_index, i)
  else if ( (aug_2022 %in% xx) | (sep_2022 %in% xx) ) L_index <- c(L_index, i)
}
tmp_list <- temporal_list
temporal_list[L_index] <- NULL
...
calculate_forecast <- function(lst){
  ...
  prophet_forecast <<- function(x, dates, h, hp){
    ...
    prophet_data <- tibble(
       ...
       checksum = dates
        ...
        mutate(covid_ind = checksum %in% seq.Date("2020-04-01" %>% as.Date(), "2020-08-01" %>% as.Date(), by = "month"),
               aug_ind = checksum %in% setdiff(seq.Date("2015-08-01" %>% as.Date(), 
                                                        "2027-10-01" %>% as.Date(),
                                                        by = "year"), 
                                               "2022-08-01" %>% 
                                               as.Date()))
    ...
    future_points <- make_future_dataframe(m, 
     ...
     future_points <- future_points %>%
        mutate(covid_ind = (future_dates %in% seq.Date("2020-04-01" %>% as.Date(), 
                                                       "2020-08-01" %>% as.Date(),
                                                       by = "month")) %>% as.numeric(),
  ...
  arima_aug <<- function(x, h, dates) {
    historic_aug <<- (dates %in% setdiff(seq.Date("2015-08-01" %>% as.Date(),
                                                  "2027-10-01" %>% as.Date(),
                                                  by = "year"), "2022-08-01" %>% 
                                         as.Date())) %>% as.numeric()
...
SalesMarkets <- income %>% pull(SalesMarket) %>% unique()
...
train_index <- c()
for(i in 1:length(forecast_list)) {
  date <- forecast_list[[i]]$Date %>% max()
  if(date < as.Date("2022-08-01")) train_index <- c(train_index, i)
}
forecast_list_train <- forecast_list[train_index %>% unique()]
forecast_list[train_index %>% unique()] <- NULL
...
hyper <- hp_search(forecast_list_train, filename = "tmp.RData", n_iter = 25)
...
train_data <- create_feat_classif_problem(forecast_list_train)
...
forecast_list_train %>% length()

# COMMAND ----------

forecast_list %>% length()
...
path_out <- paste0("/mnt/blob/hyperparameters/fforma/income/current/xgb_params_pl_sm_wd_feat.csv")
...
xgb_data_spark <- sdf_copy_to(sc, 
                                 xgb_data,
                                 overwrite = TRUE)
path_out <- paste0("/mnt/blob/hyperparameters/fforma/income/current/data_pl_sm_wd_feat.csv")
...
xgb_errors_spark <- sdf_copy_to(sc, 
                                 xgb_errors,
                                 overwrite = TRUE)
path_out <- paste0("/mnt/blob/hyperparameters/fforma/income/current/errors_pl_sm_wd_feat.csv")
...
for(i in 1:length(forecast_list)) {
  ...
  TotalOrders <- forecast_list[[i]]$xx %>% head(1) %>% as.numeric() # This is smoothed data.
...
cut_off_date <- tbl %>%
  filter(source %in% c("TotalOrders")) %>%
  group_by(DateMonth, source) %>%
  summarise(values = sum(values)) %>%
  ungroup() %>%
  filter(values < 2 * 10 ^ 6) %>%
  pull(DateMonth) %>%
  max()

tbl <- tbl %>%
  filter(DateMonth > cut_off_date)
...
tbl %>%
  #filter(source %in% c("TotalOrders", "arima_aug")) %>%
  group_by(DateMonth, source) %>%
  summarise(values = sum(values)) %>%
  print(n = 200)
...
tbl %>%
  filter(source %in% c("TotalOrders", "y_hat", "mean_forecast")) %>%
  group_by(DateMonth, source) %>%
  summarise(values = sum(values)) %>%
  ggplot()+
  geom_line(aes(x = DateMonth, y = values, color = source))
...
tbl %>%
  ...
  pivot_longer(-c(DateMonth, ProductLine, SalesMarket, TotalOrders), names_to = "source", values_to = "values") %>%
  group_by(DateMonth, source) %>%
...
tbl %>%
  pivot_wider(names_from = source, values_from = values) %>%
  pivot_longer(-c(DateMonth, ProductLine, SalesMarket, TotalOrders), names_to = "source", values_to = "values") %>%
  group_by(DateMonth, ProductLine, source) %>%
  summarise(mape = abs( sum(TotalOrders)-sum(values) ) / sum(TotalOrders),
            mae = abs( sum(TotalOrders)-sum(values) )) %>%
  ungroup() %>%
  group_by(source, ProductLine) %>%
  summarise(mape = mean(mape),
            mae = mean(mae)) %>%
  arrange(ProductLine, mape) %>%
  print(n = 150)

# COMMAND ----------

install.packages("mlflow", quiet=TRUE)
library(mlflow)
#install_mlflow()

# COMMAND ----------

mlflow_start_run(experiment_id=2651282095460863)

# COMMAND ----------

write_csv(tbl, "tbl.csv")
mlflow_log_artifact("tbl.csv")
mlflow_log_param("FOR PBI", "True")
...
write_csv(total_error, "total_error.csv")
mlflow_log_artifact("total_error.csv")
...
write_csv(month_error, "month_error.csv")
mlflow_log_artifact("month_error.csv")
...
write_csv(source_pl_error, "source_pl_error.csv")
mlflow_log_artifact("source_pl_error.csv")
