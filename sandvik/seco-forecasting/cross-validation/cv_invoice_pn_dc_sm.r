...
# MAGIC %run /Repos/alejandro.kuratomi_hernandez@secotools.com/prophet_R_secotools/sandvik/seco-forecasting/read_data/invoice/invoice_read_pn_dc_sm
...
income_pn_dc_sm <- income_pn_dc_sm %>%
  ...
  mutate(TotalOrders = replace(TotalOrders, TotalOrders < 0, 0))
...
create_temporal_list <- function(data){
  ...
  to_list <- function(tbl){
    ...
    if( length(unique(values)) == 1 ) values <- values + rnorm(length(values), sd = 0.1)
  ...
  cut_off  <- seq(0, 12) ##Every series has nr_orders = 15
  data %>%
    crossing(cut_off) %>%
    group_split(ProductNumber, SupplyingWarehouse, SalesMarket, cut_off) %>%
    lapply(., to_list) 
}

temporal_list <- SparkR::spark.lapply(income_pn_dc_sm %>% group_split(part), func = create_temporal_list) %>% unlist(recursive = FALSE)
...
L_temporal_list <- temporal_list[L_index]

# COMMAND ----------

##Aug and sep 2022 are outliers
L_index <- c()
for(i in 1:length(L_temporal_list)){
  L <- L_temporal_list[[i]]$L
  xx <- L_temporal_list[[i]]$Date %>% tail(1) %m+% months(1)
...
tmp_list <- L_temporal_list
L_temporal_list[L_index] <- NULL
...
forecast_list <- SparkR::spark.lapply(L_temporal_list %>% 
                                      create_list_for_spark(), 
...
xgb_data_spark <- sdf_copy_to(sc, 
                                 xgb_data,
                                 overwrite = TRUE)
path_out <- paste0("/mnt/blob/hyperparameters/fforma/invoice/current/data_pn_dc_sm.csv") 
...
xgb_errors_spark <- sdf_copy_to(sc, 
                                 xgb_errors,
                                 overwrite = TRUE)
path_out <- path_out <- paste0("/mnt/blob/hyperparameters/fforma/invoice/current/errors_pn_dc_sm.csv") 
...
hyper <- hp_search(forecast_list, filename = "tmp.RData", n_iter = 20)
...
path_out <- paste0("/mnt/blob/hyperparameters/fforma/invoice/current/xgb_params_pn_dc_sm.csv") 
...
for(i in 1:length(forecast_list)) {
  pn <- forecast_list[[i]]$ProductNumber
  sw <- forecast_list[[i]]$SupplyingWarehouse
...
tbl %>%
  ...
  pivot_longer(-c(DateMonth, SupplyingWarehouse, SalesMarket, ProductNumber, TotalOrders), names_to = "source", values_to = "values") %>%
  group_by(DateMonth, source) %>%
...
tbl_spark <- sdf_copy_to(sc, 
                                 tbl,
                                 overwrite = TRUE)
path_out <- path_out <- paste0("/mnt/blob/hyperparameters/fforma/invoice/current/cv_values.csv")
