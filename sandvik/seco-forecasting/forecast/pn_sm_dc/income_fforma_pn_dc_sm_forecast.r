...
path_in <- paste0("/mnt/blob/hyperparameters/fforma/income/current/data_pn_dc_sm.csv")
...
path_in <- paste0("/mnt/blob/hyperparameters/fforma/income/current/errors_pn_dc_sm.csv")
...
path_in <- paste0("/mnt/blob/hyperparameters/fforma/income/current/xgb_params_pn_dc_sm.csv")
...
forecast_tbl %>%
  group_by(Date, source) %>%
  summarise(y = sum(values, na = TRUE)) %>%
  ggplot()+
  geom_line(aes(x = Date, y = y, color = source))
...
path_out_current <- paste0("/mnt/blob/forecast/income/productnumber_dc_sm/", "current", "/pn_dc_sm.csv")
path_out_archive <- paste0("/mnt/blob/forecast/income/productnumber_dc_sm/archive/", date_today, "/pn_dc_sm.csv")
