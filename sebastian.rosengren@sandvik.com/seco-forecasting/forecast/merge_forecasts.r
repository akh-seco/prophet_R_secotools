...
income_pn_dc_sm <- spark_read_csv(sc, 
            ...
            #DateMonth = regexp_replace(TransactionDate, "-", "") %>% substr(1, 6) %>% paste0("01")
...
pn_dc_sm_forecast <- spark_read_csv(sc, 
  ...
  ungroup() %>%
  filter(ProductNumber != "03080586", Date > "2024-01-01")
...
tmp <- all_data %>%
  ...
  group_by(year, ProductLine, SalesMarket) %>%
  summarise(TotalOrders = sum(TotalOrders),
            values_adj = sum(values_adj)) %>%
  collect() %>%
  group_by(year, ProductLine, SalesMarket) %>%
  mutate(sales = sum(TotalOrders, values_adj, na.rm = TRUE)) %>%
  ungroup() %>%
  arrange(ProductLine, SalesMarket, year)

# COMMAND ----------

tmp %>%
  filter(ProductLine == "Holemaking", SalesMarket == "CN")
...
tmp %>%
  ...
  select(year, ProductLine, sales, inc) %>%
  print(n = 600)