...
temporal_list <- SparkR::spark.lapply(income_pn %>% group_split(part), func = create_list) %>% unlist(recursive = FALSE)
...
for(i in 1:length(temporal_list)){
  L <- temporal_list[[i]]$L
  if( L < 25) L_index <- c(L_index, i) #25
}
fforma_list <- temporal_list
fforma_list[L_index] <- NULL
#fforma_list <- fforma_list[sample( 1:length(fforma_list), 2000)]
...
path_in <- paste0("/mnt/blob/hyperparameters/fforma/income/current/data_pn.csv")
...
path_in <- paste0("/mnt/blob/hyperparameters/fforma/income/current/errors_pn.csv")
...
path_in <- paste0("/mnt/blob/hyperparameters/fforma/income/current/xgb_params_pn_dc.csv")
...
path_out_current <- paste0("/mnt/blob/forecast/income/productnumber_dc/", "custom", "/pn_dc.csv")
