...
# MAGIC %run /Repos/sebastian.rosengren@secotools.com/pipelines/income_read_productnumber
...
temporal_list <- SparkR::spark.lapply(income_pn %>% group_split(part), func = create_temporal_list) %>% unlist(recursive = FALSE)
...
xgb_data_spark <- sdf_copy_to(sc, 
                                 xgb_data,
                                 overwrite = TRUE)
path_out <- paste0("/mnt/blob/hyperparameters/fforma/income/current/data_pn.csv")
...
xgb_errors_spark <- sdf_copy_to(sc, 
                                 xgb_errors,
                                 overwrite = TRUE)
path_out <- path_out <- paste0("/mnt/blob/hyperparameters/fforma/income/current/errors_pn.csv")
...
path_out <- paste0("/mnt/blob/hyperparameters/fforma/income/current/xgb_params_pn_dc.csv")
