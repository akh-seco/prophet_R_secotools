...
# MAGIC %run /Repos/sebastian.rosengren@secotools.com/pipelines/invoice_read_productnumber
...
create_temporal_list <- function(data){
  ...
  data %>%
    crossing(cut_off) %>%
    group_split(ProductNumber, SupplyingWarehouse, cut_off) %>%
    lapply(., to_list) 
}

temporal_list <- SparkR::spark.lapply(invoice_pn %>% group_split(part), func = create_temporal_list) %>% unlist(recursive = FALSE)
...
xgb_data_spark <- sdf_copy_to(sc, 
                                 xgb_data,
                                 overwrite = TRUE)
path_out <- paste0("/mnt/blob/hyperparameters/fforma/invoice/current/data_pn.csv")
...
xgb_errors_spark <- sdf_copy_to(sc, 
                                 xgb_errors,
                                 overwrite = TRUE)
path_out <- path_out <- paste0("/mnt/blob/hyperparameters/fforma/invoice/current/errors_pn.csv")
...
path_out <- paste0("/mnt/blob/hyperparameters/fforma/invoice/current/xgb_params_pn_dc.csv")
...
tbl %>%
  ...
  pivot_longer(-c(DateMonth, SupplyingWarehouse, ProductNumber, TotalOrders), names_to = "source", values_to = "values") %>%
  group_by(DateMonth, source) %>%
