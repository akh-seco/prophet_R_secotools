...
# MAGIC %run /Repos/sebastian.rosengren@secotools.com/pipelines/invoice_read_pn_dc_sm
...
temporal_list <- SparkR::spark.lapply(invoice_pn_dc_sm %>% group_split(part), func = create_temporal_list) %>% unlist(recursive = FALSE)
...
xgb_data_spark <- sdf_copy_to(sc, 
                                 xgb_data,
                                 overwrite = TRUE)
path_out <- paste0("/mnt/blob/hyperparameters/fforma/invoice/current/data_pn_dc_sm_75K.csv")
...
xgb_errors_spark <- sdf_copy_to(sc, 
                                 xgb_errors,
                                 overwrite = TRUE)
path_out <- path_out <- paste0("/mnt/blob/hyperparameters/fforma/invoice/current/errors_pn_dc_sm_75K.csv")
...
path_out <- paste0("/mnt/blob/hyperparameters/fforma/invoice/current/xgb_params_pn_dc_sm_75K.csv")
