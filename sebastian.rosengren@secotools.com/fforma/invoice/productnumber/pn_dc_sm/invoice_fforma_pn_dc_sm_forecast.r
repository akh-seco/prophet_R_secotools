...
temporal_list <- SparkR::spark.lapply(invoice_pn_dc_sm %>% group_split(part), func = create_list) %>% unlist(recursive = FALSE)
...
path_in <- paste0("/mnt/blob/hyperparameters/fforma/invoice/current/data_pn_dc_sm_75K.csv")
...
path_in <- paste0("/mnt/blob/hyperparameters/fforma/invoice/current/errors_pn_dc_sm_75K.csv")
...
path_in <- paste0("/mnt/blob/hyperparameters/fforma/invoice/current/xgb_params_pn_dc_sm_75K.csv")
