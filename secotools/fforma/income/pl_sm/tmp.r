...
calculate_forecast <- function(lst){
  ...
  arima_aug <<- function(x, h, dates) {
    ...
    weird_sep <<- (dates %in% seq.Date("2022-09-01" %>% as.Date(), "2022-09-01" %>% as.Date(), by = "month")) %>% as.numeric()
    ...
    model <- forecast:::auto.arima( x, stepwise = FALSE,
                                    xreg = cbind(historic_aug, historic_covid, weird_sep) )
    ...
    xregmat <<- cbind(future_aug, future_covid, future_weird_sep)
    
    
    colnames(xregmat) <- c("historic_aug", "historic_covid", "weird_sep") 
  ...
  stl_arima <<- function(x, h, dates) {
    ...
    model <- stlm( x, method = "arima",
                                    xreg = cbind(historic_aug, historic_covid, weird_sep), stepwise = FALSE) #added stepwise = FALSE 
...
forecast_tbl %>%
  ...
  pivot_wider(names_from = source, values_from = working_days) %>%
  print(n = 200)
