...
# MAGIC %run /Repos/alejandro.kuratomi_hernandez@secotools.com/prophet_R_secotools/secotools/pipelines/income_read_pn_dc_sm
...
#}
#L_temporal_list <- temporal_list
#L_temporal_list[L_index] <- NULL
#L_temporal_list <- L_temporal_list[sample( 1:length(L_temporal_list), 75000)]
...
forecast_list <- SparkR::spark.lapply(L_temporal_list %>% create_list_for_spark(), func = calculate_forecast) %>% unlist(recursive = FALSE)
...
xgb_data_spark <- sdf_copy_to(sc, 
                                 xgb_data,
                                 overwrite = TRUE)
path_out <- paste0("/mnt/blob/hyperparameters/fforma/income/current/data_pn_dc_sm_50K.csv") ##Note 50K reading 75k in forecast
...
xgb_errors_spark <- sdf_copy_to(sc, 
                                 xgb_errors,
                                 overwrite = TRUE)
path_out <- path_out <- paste0("/mnt/blob/hyperparameters/fforma/income/current/errors_pn_dc_sm_50K.csv") ##Note 50K reading 75k in forecast
...
path_out <- paste0("/mnt/blob/hyperparameters/fforma/income/current/xgb_params_pn_dc_sm_50K.csv") ##Note 50K reading 75k in forecast
