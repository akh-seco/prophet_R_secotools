...
path_out <- paste0("/mnt/blob/hyperparameters/fforma/income/current/xgb_params.csv")
...
xgb_data_spark <- sdf_copy_to(sc, 
                                 xgb_data,
                                 overwrite = TRUE)
path_out <- paste0("/mnt/blob/hyperparameters/fforma/income/current/data.csv")
...
xgb_errors_spark <- sdf_copy_to(sc, 
                                 xgb_errors,
                                 overwrite = TRUE)
path_out <- path_out <- paste0("/mnt/blob/hyperparameters/fforma/income/current/errors.csv")
...
tbl %>%
  filter(source %in% c("TotalOrders", source[14])) %>%
