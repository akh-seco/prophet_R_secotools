...
path_in <- paste0("/mnt/blob/forecast/income/productline/current/pl_wd.csv")
...
forecast_spark <- sdf_copy_to(sc, adj_forecasts %>%
                                    ...
                                    select(-c(month, year)),
                                  overwrite = TRUE)
path_out <- paste0("/mnt/blob/forecast/income/productline/current/pl_adj_wd.csv")
spark_write_csv(forecast_spark, 
                path = path_out, 
                delimiter = ";", 
                mode = "overwrite")
path_out <- paste0("/mnt/blob/forecast/income/productline/archive/", Sys.Date() %>% str_replace_all("-", "_") ,"/pl_adj_wd.csv")
spark_write_csv(forecast_spark, 
                path = path_out, 
                delimiter = ";", 
                mode = "overwrite")
