...
# MAGIC %run /Repos/alejandro.kuratomi_hernandez@secotools.com/prophet_R_secotools/secotools/pipelines/income_read_productline_sm
...
hp_prophet <- spark_read_csv(sc, 
  ...
  collect() %>%
  filter(scaled == 0, guess == 0, august == 1) %>%
...
light_THA_features <- function(full_lst){
  ...
  vectorize <- function(lst){
    pl <- lst$ProductLine
    sm <- lst$SalesMarket
    inds <- c(1 * (ProductLines %in% c(pl)), 1 * (SalesMarkets %in% c(sm)))
    feat_tbl <- matrix(inds, nrow = 1)
    colnames(feat_tbl) <- c((ProductLines %>% str_replace(pattern = " ", replace = "_")), SalesMarkets)
...
path_out <- paste0("/mnt/blob/hyperparameters/fforma/income/current/xgb_params_pl_sm_feat.csv")
...
xgb_data_spark <- sdf_copy_to(sc, 
                                 xgb_data,
                                 overwrite = TRUE)
path_out <- paste0("/mnt/blob/hyperparameters/fforma/income/current/data_pl_sm_feat.csv")
...
xgb_errors_spark <- sdf_copy_to(sc, 
                                 xgb_errors,
                                 overwrite = TRUE)
path_out <- path_out <- paste0("/mnt/blob/hyperparameters/fforma/income/current/errors_pl_sm_feat.csv")
