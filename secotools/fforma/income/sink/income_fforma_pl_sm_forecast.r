...
path_in <- paste0("/mnt/blob/hyperparameters/fforma/income/current/data_pl_sm.csv")
...
path_in <- paste0("/mnt/blob/hyperparameters/fforma/income/current/errors_pl_sm.csv")
...
path_in <- paste0("/mnt/blob/hyperparameters/fforma/income/current/xgb_params_pl_sm.csv")
...
tbl_piv %>%
    rbind(income %>%
         ...
         summarise(values = sum(TotalOrders),
                   source = "x")) %>%
  #filter(source %in% c("x", "prophet_forecast")) %>%
  ggplot()+
...
forecast_spark <- sdf_copy_to(sc, 
                                 forecast_tbl,
                                 overwrite = TRUE)
path_out <- paste0("/mnt/blob/forecast/income/productline/current/pl_sm.csv")
spark_write_csv(forecast_spark, 
                path = path_out, 
                delimiter = ";", 
                mode = "overwrite")
path_out <- paste0("/mnt/blob/forecast/income/productline/archive/", Sys.Date() %>% str_replace_all("-", "_") ,"/pl_sm.csv")
spark_write_csv(forecast_spark, 
                path = path_out, 
                delimiter = ";", 
                mode = "overwrite")
