...
# MAGIC %run /Repos/alejandro.kuratomi_hernandez@secotools.com/prophet_R_secotools/secotools/pipelines/income_read_productline
...
calculate_forecast <- function(lst){
  ...
  arima_aug <<- function(x, h, dates, g) {
    ...
    colnames(xregmat) <- c("historic_aug", "historic_covid") ## NOTE GUESSTIMATE
...
path_in <- paste0("/mnt/blob/hyperparameters/fforma/income/current/data_wd_scaling.csv")
...
path_in <- paste0("/mnt/blob/hyperparameters/fforma/income/current/errors_wd_scaling.csv")
...
path_in <- paste0("/mnt/blob/hyperparameters/fforma/income/current/xgb_params_wd.csv")
...
forecast_tbl_scaled <- forecast_tbl

# COMMAND ----------

forecast_tbl <- forecast_tbl %>%
  left_join(pl_wd_future %>%
             rename(Date = DateMonth)) %>%
  mutate(values = values * weighted_wd)
...
forecast_spark <- sdf_copy_to(sc, 
                                 forecast_tbl,
                                 overwrite = TRUE)
path_out <- paste0("/mnt/blob/forecast/income/productline/current/pl_wd.csv")
spark_write_csv(forecast_spark, 
                path = path_out, 
                delimiter = ";", 
                mode = "overwrite")
path_out <- paste0("/mnt/blob/forecast/income/productline/archive/", Sys.Date() %>% str_replace_all("-", "_") ,"/pl_wd.csv")
spark_write_csv(forecast_spark, 
                path = path_out, 
                delimiter = ";", 
                mode = "overwrite")
