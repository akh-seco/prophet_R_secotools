...
income <- income %>%
  filter(complete.cases(.)) %>% #from 2016-01
  mutate(ProductLine = "Indexable Milling") %>%
  group_by(DateMonth, ProductLine) %>%
  summarise(TotalOrders = sum(TotalOrders),
            guesstimate = sum(guesstimate))
...
ProductLines <- c("Indexable Milling", "tmp")
...
tbl %>%
  filter(source %in% c("TotalOrders", source[8])) %>%
  group_by(DateMonth, source) %>%
  summarise(values = sum(values)) %>%
  ggplot()+
