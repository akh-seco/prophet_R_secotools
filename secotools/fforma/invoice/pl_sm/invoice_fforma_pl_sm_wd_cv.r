...
create_temporal_list <- function(data){
  ...
  library(lubridate)
  cut_off  <- seq(0, interval("2020-08-01" %>% as.Date(), end_date) %/% months(1) - 2) 
  data %>%
    crossing(cut_off) %>%
...
temporal_list <- create_temporal_list(invoice)
    ...
    prophet_data <- tibble(
       ...
       checksum = dates
        ...
        mutate(covid_ind = checksum %in% seq.Date("2020-04-01" %>% as.Date(), "2020-08-01" %>% as.Date(), by = "month"),
               aug_ind = checksum %in% seq.Date("2015-08-01" %>% as.Date(), "2022-10-01" %>% as.Date(), by = "year"))
    ...
    future_points <- make_future_dataframe(m, 
     ...
     future_points <- future_points %>%
        mutate(covid_ind = (future_dates %in% seq.Date("2020-04-01" %>% as.Date(), "2020-08-01" %>% as.Date(), by = "month")) %>% as.numeric(),
               aug_ind = (future_dates %in% seq.Date("2015-08-01" %>% as.Date(), "2022-10-01" %>% as.Date(), by = "year")) %>% as.numeric())
...
SalesMarkets <- invoice %>% pull(SalesMarket) %>% unique()
