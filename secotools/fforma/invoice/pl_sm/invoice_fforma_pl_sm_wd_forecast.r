...
# MAGIC %run /Repos/alejandro.kuratomi_hernandez@secotools.com/prophet_R_secotools/secotools/pipelines/invoice_read_productline_sm

# COMMAND ----------

path_in <- paste0("/mnt/blob/hyperparameters/prophet/invoice/current/prophet_hp.csv")
...
forecast_list <- create_list(invoice)
...
calculate_forecast <- function(lst){
    ...
    prophet_data <- tibble(
       ...
       checksum = dates
        ...
        mutate(covid_ind = checksum %in% seq.Date("2020-04-01" %>% as.Date(), "2020-08-01" %>% as.Date(), by = "month"),
               aug_ind = checksum %in% seq.Date("2015-08-01" %>% as.Date(), "2027-10-01" %>% as.Date(), by = "year"))
  ...
  arima_aug <<- function(x, h, dates) {
    historic_aug <<- (dates %in% seq.Date("2015-08-01" %>% as.Date(), "2027-10-01" %>% as.Date(), by = "year")) %>% as.numeric()
...
ProductLines <- invoice %>% pull(ProductLine) %>% unique()
...
path_in <- paste0("/mnt/blob/hyperparameters/fforma/invoice/current/data_pl_sm_wd_feat.csv")
...
path_in <- paste0("/mnt/blob/hyperparameters/fforma/invoice/current/errors_pl_sm_wd_feat.csv")
...
path_in <- paste0("/mnt/blob/hyperparameters/fforma/invoice/current/xgb_params_pl_sm_wd_feat.csv")
...
library(parallel)
library(data.table)
forecast_tbl <- rbindlist(lapply(append(forecast_list, sma_list), to_tbl, end_date = end_date)) %>% as_tibble()

# COMMAND ----------

#forecast_tbl <- forecast_list %>% lapply(to_tbl, end_date = end_date)
#library(data.table)
...
forecast_tbl <- forecast_tbl %>%
  ...
  mutate(working_days = if_else(working_days %>% is.na(), month_avg_total %>% mean(na.rm = TRUE), working_days)) %>%
  mutate(values_unscaled = values) %>%
...
forecast_tbl %>%
  #filter(SalesMarket == "CH") %>%
  group_by(Date, source) %>%
  summarise(y = sum(values),
            wd = sum(values) / sum(values_unscaled)) %>%
  print(n = 400)
...
tbl <- tibble() 
Date <- seq.Date("2022-01-01" %>% as.Date(), length.out = 60, by = "month")
...
forecast_spark <- sdf_copy_to(sc, 
                                 forecast_tbl,
                                 overwrite = TRUE)
path_out <- paste0("/mnt/blob/forecast/invoice/productline/current/pl_sm_wd.csv")
spark_write_csv(forecast_spark, 
                path = path_out, 
                delimiter = ";", 
                mode = "overwrite")
path_out <- paste0("/mnt/blob/forecast/invoice/productline/archive/", Sys.Date() %>% str_replace_all("-", "_") ,"/pl_sm_wd.csv")
spark_write_csv(forecast_spark, 
                path = path_out, 
                delimiter = ";", 
                mode = "overwrite")
