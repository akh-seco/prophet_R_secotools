...
# MAGIC     extra_configs = {"fs.azure.account.key.analyticsdatalakeprodwe.blob.core.windows.net":dbutils.secrets.get(scope = "azure_secrets", key = "blobpassword")}) #sas password valid until 2023-01
...
library(lubridate)
end_date <- str_split(dc_order_latest, "_")[[1]][3] %>% 
  substr(1,6) %>%
  paste0("01") %>%
