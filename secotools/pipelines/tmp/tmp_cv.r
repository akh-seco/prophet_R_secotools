...
income %>%
  filter(SalesMarket %in% c("IN", "IP")) %>%
  group_by(DateMonth, SalesMarket) %>%
  summarise(y = sum(TotalOrders_raw)) %>%
  ggplot()+
  geom_line(aes(x = DateMonth, y = y, color = SalesMarket))
...
temporal_list <- temporal_list %>% THA_features(n.cores=15)
...
null_index <- c()
for(i in 1:length(temporal_list)) {
  feat <- temporal_list[[i]]$features
  ff <- temporal_list[[i]]$ff
...
income %>% filter(SalesMarket == "ES", ProductLine == "Blanks") %>% glimpse()
...
tmp_list <- forecast_list[null_index %>% unique()]
