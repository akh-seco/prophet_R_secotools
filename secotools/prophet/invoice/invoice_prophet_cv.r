...
invoice <- invoice %>%
  ...
  summarise(TotalOrders = sum(TotalOrders),
            TotalOrders_scaled = sum(TotalOrders_scaled)) %>%
  ungroup()
...
i <- 1
for(lst in tmp_list) {
  slice <- list(hp = lst, prophet_data = invoice %>% filter(DateMonth <= "2021-06-01"))
  hp_list[[i]] <- slice
  i <- i+1
}
...
path_out <- paste0("/mnt/blob/hyperparameters/prophet/invoice/current/prophet_hp.csv")
...
path_out <- paste0("/mnt/blob/hyperparameters/prophet/invoice/archive/prophet_hp_", Sys.Date(), ".csv")
spark_write_csv(cv_data_spark, 
                path = path_out, 
                delimiter = ";", 
                mode = "overwrite")
